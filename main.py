...
# ════════════════════════════════════════════════════════
# FRAUD / TAMPERING DETECTION ENGINE
# ════════════════════════════════════════════════════════
RULES = {
    "odometer_reverse":    {"score": 40, "severity": "critical"},
    "odometer_gap":        {"score": 20, "severity": "high"},
    "speed_exceeded":      {"score": 25, "severity": "high"},
    "distance_abnormal":   {"score": 15, "severity": "medium"},
    "duration_too_long":   {"score": 10, "severity": "medium"},
    "trip_overlap":        {"score": 30, "severity": "critical"},
    "repeated_violations": {"score": 20, "severity": "high"},
}

MAX_SPEED_KMH   = 120   # km/h — above this is suspicious
MAX_DISTANCE_KM = 500   # km per trip — above is suspicious
MAX_DURATION_HR = 12    # hours — above triggers warning
ODO_TOLERANCE   = 2     # km — allowed gap between trips

def analyze_trips(trips: list) -> list:
    """Run all detection rules. Return list of flagged trip results."""
    results = []

    # Group by car_id
    by_car = {}
    for t in trips:
        by_car.setdefault(t["car_id"], []).append(t)

    # Driver violation counter (for repeated violations rule)
    driver_violations = {}

    for car_id, car_trips in by_car.items():
        # Sort by start_time
        sorted_trips = sorted(
            [t for t in car_trips if t.get("start_time")],
        ...
        for i, trip in enumerate(sorted_trips):
            ...
            tid       = trip["id"]
            s_odo     = trip.get("start_odometer") or 0
            e_odo     = trip.get("end_odometer")
            s_time    = trip.get("start_time")
            e_time    = trip.get("end_time")
            ...
            if e_odo is not None and float(e_odo) < float(s_odo):
                ...
                score += RULES["odometer_reverse"]["score"]

            # ── Rule 2: Odometer gap with previous trip ───────
            if i > 0:
                ...
                if prev_end is not None and s_odo:
                    ...
                    if gap > ODO_TOLERANCE:
                        ...
                        warnings.append({
                            "type": "odometer_gap",
                            "message": f"فارق عداد {gap:+.1f} كم بين نهاية الرحلة #{prev['id']} ({prev_end}) وبداية هذه الرحلة ({s_odo})",
                            "severity": sev
                        })
                        score += sc

            # ── Rules 3 & 4 & 5: Need both start + end times ─
            if s_time and e_time and e_odo and s_odo:
                try:
                    from datetime import datetime
                    fmt = "%Y-%m-%dT%H:%M:%S"
                    s_dt = datetime.fromisoformat(s_time.replace("Z",""))
                    e_dt = datetime.fromisoformat(e_time.replace("Z",""))
                    dur_hr = (e_dt - s_dt).total_seconds() / 3600
                    dist_km = float(e_odo) - float(s_odo)

                    # Rule 3: Abnormal distance
                    if dist_km > MAX_DISTANCE_KM:
                        warnings.append({
                            "type": "distance_abnormal",
                            "message": f"مسافة غير طبيعية: {dist_km:.0f} كم في رحلة واحدة (الحد: {MAX_DISTANCE_KM} كم)",
                            "severity": "medium"
                        })
                        score += RULES["distance_abnormal"]["score"]

                    # Rule 4: Speed exceeded
                    if dur_hr > 0:
                        avg_speed = dist_km / dur_hr
                        if avg_speed > MAX_SPEED_KMH:
                            sev = "critical" if avg_speed > 200 else "high"
                            warnings.append({
                                "type": "speed_exceeded",
                                "message": f"سرعة متوسطة مشبوهة: {avg_speed:.0f} كم/ساعة (الحد: {MAX_SPEED_KMH} كم/ساعة)",
                                "severity": sev
                            })
                            score += RULES["speed_exceeded"]["score"]

                    # Rule 5: Duration too long (ended trip)
                    if dur_hr > MAX_DURATION_HR:
                        warnings.append({
                            "type": "duration_too_long",
                            "message": f"مدة الرحلة طويلة جداً: {dur_hr:.1f} ساعة (الحد: {MAX_DURATION_HR} ساعة)",
                            "severity": "medium"
                        })
                        score += RULES["duration_too_long"]["score"]
                except Exception:
                    pass

            # Rule 5b: Still active too long
            elif s_time and not e_time:
                try:
                    from datetime import datetime, timezone
                    s_dt = datetime.fromisoformat(s_time.replace("Z",""))
                    now  = datetime.utcnow()
                    dur_hr = (now - s_dt).total_seconds() / 3600
                    if dur_hr > MAX_DURATION_HR:
                        warnings.append({
                            "type": "duration_too_long",
                            "message": f"الرحلة لا تزال جارية منذ {dur_hr:.1f} ساعة — لم يتم الإنهاء",
                            "severity": "medium"
                        })
                        score += RULES["duration_too_long"]["score"]
                except Exception:
            ...
            # ── Rule 6: Trip overlap (same car) ───────────────
            if s_time and e_time and i > 0:
                prev = sorted_trips[i - 1]
                if prev.get("end_time") and prev["end_time"] > s_time:
                    warnings.append({
                        "type": "trip_overlap",
                        "message": f"تداخل زمني مع الرحلة #{prev['id']} — مستحيل لنفس المركبة",
                        "severity": "critical"
                    })
                    score += RULES["trip_overlap"]["score"]

            # Track driver violations for repeated rule
            if warnings and driver_id:
                driver_violations[driver_id] = driver_violations.get(driver_id, 0) + len(warnings)

            if warnings:
                risk_level = "critical" if score >= 40 else "high" if score >= 20 else "medium" if score >= 8 else "low"
                results.append({
                    "trip_id":    tid,
                    "driver_id":  driver_id,
                    "car_id":     car_id,
    ...
    # ── Rule 7: Repeated violations per driver ────────────────
    for r in results:
        did = r.get("driver_id")
        if did and driver_violations.get(did, 0) >= 3:
            already = any(w["type"] == "repeated_violations" for w in r["warnings"])
            if not already:
                r["warnings"].append({
                    "type": "repeated_violations",
                    "message": f"هذا السائق لديه {driver_violations[did]} مخالفات متكررة — مستوى خطر مرتفع",
                    "severity": "high"
                })
                r["risk_score"] += RULES["repeated_violations"]["score"]
                if r["risk_score"] >= 40:
